// $Id: SG_T.h 160 2007-06-22 15:21:10Z mburo $
...
CSGGameBase* CSG<TRules>::PGame(const string& idg) {
  typename map<string, TGame>::iterator i = idToGame.find(idg);
  if (i==idToGame.end())
    return NULL;
  else
    return &((*i).second);
...
typename CSG<TRules>::TRequest* CSG<TRules>::PRequest(const string& idr) {
  typename map<string,TRequest>::iterator i=idToRequest.find(idr);
...
void CSG<TRules>::BaseRequestDelta(const TMsgRequestDelta& msg) {
  typename map<string,TRequest>::iterator i=idToRequest.find(msg.idr);
  if (msg.fPlus) {
...
bool CSG<TRules>::BaseUpdate(const TMsgUpdate& msg) {
  ...
  typename map<string,TGame>::iterator i=idToGame.find(msg.idg);
  if (i!=idToGame.end()) {
