...
void Stat::te_handle( sint4 /*Mssg*/, uint4 Time )
{
  ...
  vc_var.histo_requests .update( n_request.current() );
  vc_var.histo_matches  .update( n_match.  current() );
  vc_var.modified();
  io_last = System::real_time();
  ...
  if ( warn_ptr == 0 ) {
    ...
    if ( svc_ready ) { vc_con << VCFL; svc_ready = false; }
  } else {		      
    if (!svc_ready ) { vc_con << VCFL; svc_ready = true; }
  }
...
ostream& Stat::print( ostream& os ) const
{
  ...
  os << "Users       : " << n_user    << EOL;
  os << "Requests    : " << n_request << EOL;
  os << "Matches     : " << n_match   << EOL;
