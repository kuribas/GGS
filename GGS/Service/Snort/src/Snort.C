// $Id: Snort.C 160 2007-06-22 15:21:10Z mburo $
...
bool Snort::read_pos_ggf(istream &is) { return pos.read_ggf(is); }

void Snort::write_pos_ggf(ostream &os, bool one_line) const {
  pos.write_ggf(os, one_line);
}

void Snort::write_pos_txt(ostream &os) const {
  pos.write(os);
}

void Snort::init_pos(sint4 board_type, sint4 rand_type) {
  pos.init(board_type, rand_type);
}

RegularBoardGame::TURN Snort::color_to_move() const
{
  return RegularBoardGame::TURN( pos.get_to_move() );
}

bool Snort::legal_move(ostream &err, const string &mvs) {
...
void Snort::do_move(const string &mvs) {
...
void Snort::make_random_move() { pos.make_random_move(); }
void Snort::make_good_move() { pos.make_good_move(); }
bool Snort::game_finished() const { return pos.is_finished(); }

sint4 Snort::blacks_result(bool) const { return pos.blacks_result(); }

sint4 Snort::max_result() const {
...
RegularBoardGame *RegularBoardGame::new_game() { return new Snort; }
...
const char * const RegularBoardGame::GAME_NAME = "Snort";

const char * const RegularBoardGame::LOGIN_SERVICE = "/snort";
const char * const RegularBoardGame::LOGIN_SYSTEM  = " s.n.o.r.t. ";
